...
static int rt5630_write_vodsp_reg(struct snd_soc_codec *codec, unsigned int vodspreg, unsigned int value)
{
	...
	ret = rt5630_wait_vodsp_i2c_done(codec);
	if (ret)
		return ret;
...
static int realtek_ve_mode_set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	int mode = codec->read(codec, VIRTUAL_REG_FOR_MISC_FUNC);
	RT5630_DEBUG("realtek_ve_mode_set:mode=%x, uctrl=%ld\n", mode,ucontrol->value.integer.value[0]);
...
#endif  //USE_DAPM_CTRL, we not defined USE_DAPM_CTRL
#if !USE_DAPM_CTRL

#if 0
enum OUTPUT_DEVICE_MASK
{
	SPK_OUTPUT_MASK = 1,
	HP_OUTPUT_MASK,
};
...
{
	struct snd_soc_codec *codec = codec_dai->codec;
	struct rt5630_priv *rt5630 = snd_soc_codec_get_drvdata(codec);
	RT5630_DEBUG("freq=%d\n",freq);
			...
			struct snd_soc_dai *dai)
{
	...
	if (stream == SNDRV_PCM_STREAM_CAPTURE) {
		list_for_each_entry(w, &codec->driver->dapm_widgets, list)  //@@@@@
		{
			if (!w->sname)
				continue;
...
static void rt5630_hifi_shutdown(struct snd_pcm_substream *substream, struct snd_soc_dai *codec_dai)  //@@@@@
{
	struct snd_soc_codec *codec = codec_dai->codec;
	int stream = substream->stream;
	RT5630_DEBUG("stream=%d\n",stream);	
...
#ifdef CONFIG_SND_DOVE_SOC_AC97_AVD1
...
struct snd_soc_dai_driver rt5630_dai[] = {
	/*hifi codec dai*/
	{
		.name = "rt5630-hifi",
		.id = 1,
...
static int rt5630_init(struct snd_soc_codec *codec)   //@@@@@
{
	int ret = 0;
...
static struct snd_soc_codec *rt5630_codec;
static struct i2c_client *rt5630_client;

static int rt5630_probe(struct snd_soc_codec *codec)
{    //the platform_device is belongs to snd_soc_card->dev; this is set in function mv88fx_snd_probe()
	...
	rt5630_codec = codec ;
	//snd_soc_codec_set_cache_io(codec, 8, 16, SND_SOC_I2C);
	i2c_set_clientdata(rt5630_client, codec);
	codec->control_data = rt5630_client;
	codec->hw_write = (hw_write_t)i2c_master_send;

	ret = rt5630_init(codec);
	...
	snd_soc_codec_set_drvdata(codec, rt5630);
...
static int rt5630_remove(struct snd_soc_codec *codec)
{
	struct snd_soc_card *card = codec->card;
	struct platform_device *pdev;
	struct rt5630_priv *rt5630 = snd_soc_codec_get_drvdata(codec);
...
static int rt5630_suspend(struct snd_soc_codec *codec)
{
	int i;
	u8 data[3];
	RT5630_DEBUG("");
...
static int rt5630_resume(struct snd_soc_codec *codec)
{
	int i;
	u8 data[3];
	u16 *cache = codec->reg_cache;
...
static struct snd_soc_codec_driver soc_codec_dev_rt5630 = {
	.probe = rt5630_probe,
	.remove = 	rt5630_remove,
	.suspend = 	rt5630_suspend,
	.resume =	rt5630_resume,
	//.reg_cache_size = 0xff,
	.reg_cache_size = sizeof(rt5630_reg),
	.reg_word_size = sizeof(u16),
	.reg_cache_step=2,
	.controls = rt5630_snd_controls,
	.num_controls = ARRAY_SIZE(rt5630_snd_controls),
...
static int __init_refok rt5630_i2c_probe(struct i2c_client *i2c_client, const struct i2c_device_id *id)
{
	//int i;
	//for (i = 0; i < ARRAY_SIZE(rt5630_dai); i++)
	//	rt5630_dai[i].dev = &pdev->dev;
	rt5630_client = i2c_client;
	
	return snd_soc_register_codec(&i2c_client->dev, &soc_codec_dev_rt5630, rt5630_dai, ARRAY_SIZE(rt5630_dai));
}


static int __devexit rt5630_i2c_remove(struct i2c_client *i2c_client)
{
	struct snd_soc_codec *codec = rt5630_codec;
...
static const struct i2c_device_id rt5630_id[] = { {"rt5630", 0},  {} };
MODULE_DEVICE_TABLE(i2c, rt5630_id);

static struct i2c_driver rt5630_i2c_driver = {
	.probe		= rt5630_i2c_probe,
	.remove		= __devexit_p(rt5630_i2c_remove),
	.id_table = rt5630_id,  
	.driver		= {
		.name	= "rt5630-codec",
		.owner	= THIS_MODULE,
...
{
	return i2c_add_driver(&rt5630_i2c_driver);
}
